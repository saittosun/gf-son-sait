...
export class SelectComponent implements OnInit {
  ...
  ngOnInit() {
    this.store
      ...
      .pipe(
        take(1),
        map((bookigState) => {
          return {searchPhaseModel: bookigState.searchPhaseModel}
        })
      )
      .subscribe((state) => {
        ...
        this.searchForm = clone(state.searchPhaseModel.searchForm);
      });
  ...
  select(dicIndex: number, tripIndex: number){
    ...
    console.log(this.selectedTrips.filter(trip => trip.tripIndexInSearchResponse >=0).length);
    this.readyForCalculation = this.selectedTrips.filter(trip => !(trip.tripIndexInSearchResponse >=0)).length === 0;
  }

  handleNewDateSearch(dicIndex: number, direction: boolean){
    const newDate = direction ? this.routeDateAlternatives[dicIndex].nextDate : this.routeDateAlternatives[dicIndex].previousDate;
    if (!newDate) return;
    this.searchForm.passengerCount = this.passengerCount;
    this.searchForm.vehicleCount = this.vehicleCount;
    this.searchForm.routes[dicIndex].date = new Date(newDate);
    this.isLoading = true;
    this.databaseHandlerService.postSearchRequest(this.searchForm).subscribe(
      (searchResponse) => {
        this.searchForm.routes[dicIndex].date  = new Date(new Date(newDate).getTime() - (60*60*24*1000) );
  ...
  handleGenderChange(obj: { index: string | number; gender: Gender; }){
    ...
    this.selectedGenders[obj.index] = obj.gender;
